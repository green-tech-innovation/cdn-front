...
export default {
  ...
  methods: {
    loadList() {
      fetch(Helper.route('sector/gallerys'), Helper.requestOptions())
      .then(response => {
    ...
    editItem() {
      ...
      console.log(formData.get("name"), 'id', this.gallery.id);
              ...
              <table class="table align-middle mb-0">
                ...
                <tbody>
                  <tr v-for="(gallery, i) in gallerys" :key="gallery.id">
                    <th scope="row">{{ i + 1 }}</th>
                    <td>
                      <router-link :to="'/sector/gallery/'+gallery.id">{{ gallery.name }}</router-link>
                    </td>
